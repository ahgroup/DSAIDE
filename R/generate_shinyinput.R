...
generate_shinyinput <- function(model_function, is_mbmodel, otherinputs = NULL, packagename)
{
    ...
    ###########################################
    #this is done for a function adhering to mbmodel structure
    if (is_mbmodel)
    {
        ip = formals(model_function) #get all input arguments for function
        #turn pair-list into a vector of inputs
        ip = c(eval(ip$vars),eval(ip$pars),eval(ip$times))
