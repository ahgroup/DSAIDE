#' SEIRS model
#' 
#' @description A 4 compartment model with natural births and deaths and waning immunity.
#' 
...
#' @param bE : level of infectiousness of exposed : numeric
#' @param bI : level of infectiousnes of symptomatic : numeric
#' @param gE : rate at which hosts move from E to I (inverse of exposed duration) : numeric
#' @param gI : rate at which hosts recover (inverse of symptomatic duration) : numeric
#' @param w : rate at which immunity wanes (inverse of immunity duration) : numeric
#' @param n : births : numeric
#' @param m : natural deaths (inverse of life-span) : numeric
...
#' result <- simulate_SEIRS_model_stochastic() 
#' # To choose values other than the standard one, specify them like this:  
#' result <- simulate_SEIRS_model_stochastic(S = 2000,E = 2,I = 2,R = 0) 
...
simulate_SEIRS_model_stochastic <- function(S = 1000, E = 1, I = 1, R = 0, bE = 0, bI = 0.001, gE = 1, gI = 1, w = 1, n = 0, m = 0, tstart = 0, tfinal = 100, dt = 0.1, rngseed = 123) 
{ 
  #Block of ODE equations for adaptivetau 
  SEIRS_model_fct <- function(y, parms, t) 
  {
  ...
  simout = adaptivetau::ssa.adaptivetau(init.values = varvec, transitions = transitions,
                  	 	 	 rateFunc = SEIRS_model_fct, params = parvec, tf = tfinal) 
