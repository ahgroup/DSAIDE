...
* Recovered individuals are immune to reinfection and can lose their immunity at some rate. 
...
$$\dot S = -S (b_P P + b_A A + b_I I) + wR $$ 
$$\dot P = S (b_P P + b_A A + b_I I) - g_P P $$ 
$$\dot A = f g_P P - g_A A $$ 
$$\dot I = (1-f) g_P P - g_I I $$ 
$$\dot R = g_A A + (1-d) g_I I - wR$$
$$\dot D = d g_I I $$
...
* Set the model parameters such that it corresponds to the following setting: 
    ...
    * Assume that there are no asymptomatic infections, nobody dies due to disease, and immunity does not wane.
