...
##Overview {#shinytab1}
This app allows you to explore a very basic infectious disease simulation. The main goal is to provide familiarity with the overall setup and ideas behind using these simulations, and how to run them. Read about the model in the "Model" tab. Then do the tasks described in the "What to do" tab.
...
###Some notes
* For many of the models, the underlying equations are shown. If the equations don't display properly, try opening the app with a different browser.
* You will see both the term *host(s)* and *individual(s)* used interchangeably. In general, while we most often think of humans, the host can be any kind of animal (or even a plant).
...
* In contrast, the quantities that are usually fixed for a given scenario are called parameters. For this model, those are the infection rate _b_ and the recovery rate _g_.
...
* There are no fixed rules concerning the naming of variables and parameters. Compartments (e.g. SIR) tend to be labeled very similar by different researchers, parameter labels are much more different. Always check carefully for a given paper/model what the meaning of each parameter is.



##What to do {#shinytab3}
For the following tasks, it is assumed that the model runs in units of days.

###Task 1: 
* Run the simulation with a population size of 1000, 1 initially infected host, simulation duration 100 days, recovery rate _g_=0.5 per day, and infectiousness _b_0.001. You will get an outbreak of some - currently unspecified - infectious disease. 
