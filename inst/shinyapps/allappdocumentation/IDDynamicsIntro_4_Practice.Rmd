...
The SIR model is very basic. It can of course be extendend by introducing further compartments. For instance we could stratify according to gender, which would give us 2 sets of SIR compartments, one for males and one for females. We will see this and other ways of making models more detailed later in the course.
...
As with the compartments, we can of course extend the model and allow other processes to occur. For instance we could allow for natural births and deaths, waning immunity, deaths due to disease, etc. We will do that soon. For now, we keep the model as simple as possible.
...
* In general, the entities that change in our model (i.e. here the number of individuals in compartments S, I and R) are called variables: They are variable and change during the simulation. In contrast, the quantities that are usually fixed for a given scenario are called parameters. For this model, those are the infection rate $\beta$ and the recovery rate $\gamma$.
...
###Note:
You might not yet fully understand exactly why you get the dynamics you see. That's ok. This simulation exercise is to "get you thinking", we'll get deeper into the "whys" later in the course. For now, just for yourself, try to figure out as much as possible why you get what you get. 
