...
#shows a 'running simulation' message
  ...
  withProgress(message = 'Running Simulation', value = 0,
        ...
        simresult <- simulate_heterogeneity(S10 = S10, I10 = I10, S20 = S20, I20 = I20, tmax = tmax,  b11 = b11,
                                            b12 = b12, b21 = b21, b22 = b22, g1 = g1 , g2 = g2, w1 = w1, w2 = w2)
                                                     
          })
  
    colnames(simresult) = c('xvals',"S1","I1","R1","S2","I2","R2") 
    
#reformat data to be in the right format for plotting
#each plot/text output is a list entry with a data frame in form xvals, 
#yvals, extra variables for stratifications for each plot
...
#the following are for text display for each plot
    ...
    result[[1]]$finaltext = 'Numbers are rounded to 2 significant digits.' #text can be added here which will be passed through 
                                                                           #to generate_text and displayed for each plot
...
#section to add buttons
  ...
  fluidRow(
    ...
    column(6,
           ...
           fluidRow(
             column(6,
                    numericInput("b12", "Rate of transmission from type 1 to type 2 (b12)", min = 0, max = 0.01, value = 0, step = 0.0001  )
             ),
             column(6,
                    numericInput("b21", "Rate of transmission from type 2 to type 1 (b21)", min = 0, max = 0.01, value = 0, step = 0.0001  )
             )
           ), #close fluidRow structure for input
           fluidRow(
             column(6,
