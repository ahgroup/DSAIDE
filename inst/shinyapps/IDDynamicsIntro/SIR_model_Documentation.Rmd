...
* The app lets you choose starting time, final time and time step. Starting time generally does not matter and is almost always set to 0. For models like this one based on differential equations, the time step does not affect the internal time steps taken by the numeric solver, only the times for which results are returned. Only for 
...
* For this app, the underlying function running the simulation is called `simulate_SIR_model_ode.R`. You can call this function directly, without going through the shiny app. Type `?simulate_SIR_model_ode` into the R console for more information (you need to exit the graphical interface first or start a new R session). If you go that route, you need to use the results returned from this function and produce useful output (such as a plot) yourself. 
