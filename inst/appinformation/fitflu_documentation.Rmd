...
#########################
tid = tid + 1
tasktext = "Ideally, with enough iterations, all solvers should reach the best fit with the lowest possible SSR. In practice, that does not always happen. Often it depends on the starting conditions. Let's explore whether starting values matter. For a single iteration (no fitting), with the starting conditions we have chosen so far, we get an SSR=7.3E12. In the previous task, you found that using solver 3 for 200 iterations, you get SSR=2.3E10 (also note the values for the best fit parameters).
