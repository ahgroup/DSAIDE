...
nrec = 1 # number of items to record
out_records = c("Number susceptibles remaining at end of quarantine simulation.")
out_types = c("Rounded_Integer")
out_notes = c("Report the rounded integer")
outcomes = data.frame( TaskID = rep(tid,nrec),
                       TaskText = rep(tasktext,nrec),
...
########################
tid = tid + 1
tasktext = "Set parameter values back to what they were at the start of the previous task. Now, we envision a scenario where we can quarantine everyone who has become infected, independent of symptom status. We assume quarantining reduces transmission by half for anyone quarantined. Implement such a scenario, again run the simulation and record the number of susceptibles left at the end."
nrec = 1 # number of items to record
...
########################
tid = tid + 1
tasktext = "Set parameter values back to what they were at the start of the previous task. Then change transmission terms such that bP=0.001, bA=0.002, bI=0.001. Can you think of a pathogen where asymptomatic individuals might be more infectious than symptomatic individuals? Some STI might fall into this category. Pathogens that require close contact could also be of that type if symptomatic are at home sick and can't spread the disease. Run the simulation with those settings, you should see an outbreak with around 203 susceptible left. Now assume that we can administer a drug which reduces symptoms in people, but doesn't change their infectiousness. We mimic this by assuming the fraction that are asymptomatic increases to 75%. What do you expect this will do to the overall outbreak size? Run the simulation to check."
nrec = 1 # number of items to record
out_records = c("Number susceptibles remaining at end of simulation.")
out_types = c("Rounded_Integer")
out_notes = c("Report the rounded integer")
...
########################
tid = tid + 1
tasktext = "Set pathogen decay rate back to what it was at the beginning of the previous task. Now we assume that we have an intervention that reduces shedding of pathogen by infected (e.g. through better access to sanitation). To mimic this, half the rate of environmental shedding by symptomatic individuals. Run the simulation."
nrec = 2 # number of items to record
out_records = c("Number susceptibles remaining at end of simulation.",
                "Max/peak of E during simulation.")
out_types = rep("Rounded_Integer",nrec)
