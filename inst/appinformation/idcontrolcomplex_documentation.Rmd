...
* Set the model parameters such that it corresponds to the following setting: 
    ...
    * Assume that there are no births or natural deaths occurring.
...
### Task 2: 
* Let's now assume that 50% of infected hosts are asymptomatic (_f_) and that the duration of the asymptomatic stage (_gA_) is the same as the symptomatic stage (_gI_).
...
* Assume that quarantining reduces infectiousness by half. 
  * First, we envision a scenario where we can only detect and quarantine individuals that show symptoms. Implement such a scenario, run the simulation.
...
**Record**
* The number of susceptibles left at the end when only symptomatic patients are quarantined 
* The number of susceptibles left at the end when all infected patients are quarantined 
...
* Now assume that we can administer a drug. It will likely only be given to symptomatic individuals. 
   * First assume that the drug reduces the infectiousness of symptomatic individuals by half. Run the simulation. 
   * Now assume that the drug _also_ reduces the duration of the symptomatic period from a month (30 days) to 20 days. Run the simulation. 

**Record**
* The number of susceptibles left at the end when symptomatic patients are treated and infectiousness is reduced 
* The number of susceptibles left at the end when symptomatic patients are treated and infectiousness and duration are reduced 
...
* Now assume that the drug reduces the duration of the symptomatic period from a month (30 days) to 20 days. Run the simulation. 
* Now assume that the drug also cuts infectiousness by half. Run the simulation. 

**Record**
* The number of susceptibles left at the end when symptomatic patients are treated and duration is reduced
* The number of susceptibles left at the end when symptomatic patients are treated and duration and infectiousness are reduced

### Task 5: 
* Set everything as in task 1. Then turn on environmental shedding by symptomatic individuals and decay both at rates of 1 per month. Run the simulation. 

**Record**
Number of susceptibles at the end
...
* Let's assume different interventions that affect the environmental transmission.
  * First, we consider an intervention that leads to a 50% increased pathogen clearance from the environment. Run the simulation.
  * Instead of faster clearance, assume a 50% reduced rate of infection from the environment. Run the simulation.

**Record**
* The number of susceptibles left at the end when treatment increases pathogen clearance from the environment
* The number of susceptibles left at the end when treatment decreases infectiousness of pathogen in the environment.
...
* Assume we sprayed against mosquitoes and it reduced the population size by 90%. Set the initial vector population to that value. Run the simulation.

**Record**
The total number infected when vector population is reduced by 90% 

### Task 8: 
* Instead of killing vector populations, we now consider a reduction in transmission, e.g. due to the use of bed nets.
* Set everything as in task 7 (minus the intervention). 
* Assume that an intervention only reduces transmission _to_ vectors by half. Run the simulation, observe.
* Now assume that an intervention only reduces transmission _from_ vectors by half. Run the simulation, observe.
