...
nrecord = 11 #number of outcomes to record
#set up task tables
...
# Task 2
tid = 2
alltasks[tid,"TaskText"] = "A) Assume now that half of the infected are asymptomatic. Don't change any other assumption. What do you expect to get for the number/fraction of S/I/R at the end of the outbreak and the time at which the outbreak peaks?
B) Run another simulation, record the same values as above. Compare your expectations with the results. How do they agree/disagree? Does it make sense? Anything surprising happening?"

# Record for task 2
nrec = 2 # number of items to record
reclist = list()
reclist$rectext = c("Day the outbreak peaked",
...
# Task 3 
tid = 3
alltasks[tid,"TaskText"] = "A) Now assume that the asymptomatics transmit at the same rate as the symptomatic. Change _b~A~_ accordingly. Leave everything else as before. How do you expect the results to change? (Try to make as precise/quantitative a prediction as you can). Run another simulation, compare your expectations with the results. How do they agree/disagree? Does it make sense? Anything surprising happening?"

# Record for task 3
nrec = 2 # number of items to record
reclist = list()
reclist$rectext = c("Day the outbreak peaked",
                    "Total number recovered at end of outbreak")
reclist$recnote = c("Report the rounded integer",
                    "Report the rounded integer")
reclist$rectype = c("Numeric",
                    "Numeric")
...
# Task 4
tid = 4
alltasks[tid,"TaskText"] = "A) Next, let's assume that half the symptomatic infected die. Change the corresponding parameter in the model to reflect this new assumption. Leave everything as in task 3.How do you expect the results to change?
B) Run another simulation, record the same values as above. Compare your expectations with the results. How do they agree/disagree? Does it make sense? Anything surprising happening? "
...
# Task 5
tid = 5
alltasks[tid,"TaskText"] = "A) Set all transmission parameters to 0.0005, leave everything else as before. What do you expect will happen to the peak of _P_ and _I_ as well as the total fraction of population ever infected (i.e., _R_ + _D_ at the end of simulation), compared to the previous task? (use these values for the recorded max values below)
B) Further explore how changes in the infectiousness of the different groups (_b~P~_, _b~A~_, _b~I~_) and the average time a person spends in each of those states (_g~P~_, _g~A~_, _g~I~_) affects the infection dynamics. Every time, think about what you expect to get, then run the simulation, compare your expectations with the results. Then make sense of it."

# Record for task 5
nrec = 2 # number of items to record
reclist = list()
reclist$rectext = c("Max number of pre-symptomatic (Part A)",
                    "Max number of infected (Part A)")
reclist$recnote = c("Report the rounded integer",
                    "Report the rounded integer")
reclist$rectype = c("Numeric",
                    "Numeric")
...
# Task 6
tid = 6
alltasks[tid,"TaskText"] = "A) Using the settings from task 5 part A, now assume that 90% of infections are asymptomatic. How do things change?
B) Further explore how changes in the fraction becoming asymptomatic and fraction dying does (or does not) affect the infection dynamics. Every time, think about what you expect to get, then run the simulation, compare your expectations with the results. Then make sense of it."
