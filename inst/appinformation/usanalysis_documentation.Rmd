...
For the tasks, we assume the model is run in units of weeks.
...
#########################
tid = tid + 1
tasktext = "Let's continue with the exploration of the last task, to illustrate an important point. Set ranges for birth rate to 10 - 20 and death rate range 0.1 - 0.2. Run 50 simulations with random seed at 100. Look at the values at final steady state (and record the one requested).
...
nrec = 2 # number of items to record
out_records = c("The correlation between infection peak and _b_, 100 samples, rngseed = 100",
            "The correlation between final number of susceptibles and _m_, 100 samples, rngseed = 100")
...
nrec = 1 # number of items to record
out_records = c("Correlation between _g_ and the outbreak peak, with _m_ being varied.")
out_types = rep("Numeric",nrec)
out_notes = rep("Report rounded to 2 digits (hundredths)",nrec)
outcomes = data.frame( TaskID = rep(tid,nrec),
                       TaskText = rep(tasktext,nrec),
